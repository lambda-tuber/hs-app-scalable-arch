...
module ASA.UI.Response.ControlSpec (spec) where
...
import qualified ASA.UI.Response.Control as SUT
import qualified ASA.UI.Response.Type as SUT
  ...
  describe "runApp" $ do
    context "when AppData default" $ do
      it "should be 10" $ \ctx -> do 
        ...
        STM.atomically $ STM.writeTQueue queue 100
